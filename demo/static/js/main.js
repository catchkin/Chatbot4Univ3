...
function greet() {
    setTimeout(function () {
        return sendMessage("데모에 오신걸 환영합니다. 궁금한 내용 물어보세요", 'left');
    }, 1000);
...
function requestChat(messageText, url_pattern) {
    $.ajax({
        ...
        success: function (data) {
            ...
            if (state === 'SUCCESS') {
                //answer = data['query'] + data['answer'] + data['score']
                //return sendMessage(answer, 'left');
...
function onSendButtonClicked() {
    ...
    if (messageText.includes('안녕')) {
        setTimeout(function () {
            return sendMessage("안녕하세요. 저는 데모입니다.", 'left');
        }, 1000);
